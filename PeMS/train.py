...
parser.add_argument(
    "--train_ratio", type=float, default=0.7, help="training set [default : 0.7]"
)
parser.add_argument(
    "--val_ratio", type=float, default=0.1, help="validation set [default : 0.1]"
)
parser.add_argument(
...
parser.add_argument(
    "--model_file", default="data/GMAN(PeMS)", help="save the model to disk"
)
...
utils.log_string(
    log, "val              %.2f\t\t%.2f\t\t%.2f%%" % (val_mae, val_rmse, val_mape * 100)
)
utils.log_string(
    log,
    "test             %.2f\t\t%.2f\t\t%.2f%%" % (test_mae, test_rmse, test_mape * 100),
)
...
for q in range(args.Q):
    ...
    utils.log_string(
        log,
        "step: %02d         %.2f\t\t%.2f\t\t%.2f%%" % (q + 1, mae, rmse, mape * 100),
    )
